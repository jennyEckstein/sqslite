...
const { boolean, coerce, defaulted, object, optional, string, struct } = require('superstruct');
const { ErrorWithCode } = require('./errors');

const structs = {
  delaySeconds: struct('delaySeconds', (delaySeconds) => {
  ...
  kmsDataKeyReusePeriodSeconds: struct(
  ...
  maximumMessageSize: struct('maximumMessageSize', (maximumMessageSize) => {
  ...
  messageRetentionPeriod: struct('messageRetentionPeriod', (messageRetentionPeriod) => {
  ...
  queueName: struct('queueName', (queueName) => {
  ...
  receiveMessageWaitTimeSeconds: struct(
  ...
  redrivePolicy: struct('redrivePolicy', (redrivePolicy) => {
  ...
  visibilityTimeout: struct('visibilityTimeout', (visibilityTimeout) => {
...
const assertMessage = (params) => {
  return coerce(params, Message);
};

const assertQueue = (params) => {
  return coerce(params, Queue);
};
