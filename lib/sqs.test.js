...
const { clearQueues, createQueue, listQueues, receiveMessage, sendMessage } = require('./sqs');
...
describe('sqs', () => {
  ...
  describe('receiveMessage', () => {
    ...
    test('successfully receive message', () => {
      createQueue({ QueueName: 'foo-bar' });
      sendMessage({ MessageBody: 'foo', QueueUrl: 'https://queue.amazonaws.com/queues/foo-bar' });
      expect(receiveMessage({ QueueUrl: 'https://queue.amazonaws.com/queues/foo-bar' })).toEqual([
        {
          Body: 'foo',
