...
describe('sqs', () => {
  ...
  describe('sendMessage', () => {
    ...
    test('Change availability for duplicate messages by 5 minutes', () => {
      ...
      expect(secondAvailableSince - secondSentTimeStamp).toEqual(FIVE_MINUTES);
  ...
  describe('sendMessageBatch', () => {
    ...
    test('successfully send message batch with duplicates, fifo queue', () => {
      const QueueName = 'foo-bar.fifo';
      const QueueUrl = `http://localhost:3000/queues/${QueueName}`;
      createQueue(
        { Attributes: { ContentBasedDeduplication: true, FifoQueue: true }, QueueName },
        'localhost:3000'
      );
      expect(
        sendMessageBatch({
          QueueUrl,
          messages: [
            { Id: 'id1', MessageBody: 'foo', MessageGroupId: '123' },
            { Id: 'id2', MessageBody: 'foo', MessageGroupId: '123' }
          ]
        })
      ).toEqual([
        {
          Id: 'fafe1b60c24107ccd8f4562213e44849',
          MDOfMessageBody: 'acbd18db4cc2f85cedef654fccc4a4d8',
          MessageId: expect.any(String)
        },
        {
      ...
      const queueState = getQueueState(QueueUrl).messages;
      const firstMessageAttributes = queueState[0].Attributes;
      const duplicateMessageAttributes = queueState[1].Attributes;

      expect(firstMessageAttributes.AvailableSince).toEqual(firstMessageAttributes.SentTimestamp);
      expect(duplicateMessageAttributes.AvailableSince).not.toEqual(
        duplicateMessageAttributes.SentTimestamp
      );
      expect(
        duplicateMessageAttributes.AvailableSince - duplicateMessageAttributes.SentTimestamp
      ).toEqual(FIVE_MINUTES);
    });
