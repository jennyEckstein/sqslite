    ...
    test('throws an error when deadLetterQueue exists but does not much fifo/not fifo of the queue', () => {
      ...
      const params = {
        Attributes: {
          FifoQueue: 'true',
          RedrivePolicy:
            '{"deadLetterTargetArn":"arn:aws:sqs:us-east-1:123:dead-letter-queue","maxReceiveCount":5}'
        },
    ...
    test('successfully set redrive policy', () => {
      ...
      const params = {
        Attributes: {
          FifoQueue: 'true',
          RedrivePolicy:
            '{"deadLetterTargetArn":"arn:aws:sqs:us-east-1:123:dead-letter-queue1.fifo","maxReceiveCount":5}'
        },
        QueueName: 'foo-with-dlq.fifo'
      };
