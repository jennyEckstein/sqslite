...
describe('sqs', () => {
  describe('createQueue', () => {
    ...
    test('create queue with the same name and identical RedrivePolicy', () => {
      ...
      createQueue(queueParams1);
      expect(createQueue(queueParams2)).toEqual('foo-bar');
    });
    ...
    test('successfully create queue with tags', () => {
      ...
      const result = createQueue(params);
      expect(result).toEqual('foo-bar-with-tags');
    });
  ...
  describe('listQueues', () => {
    ...
    test('list all queues when list has queues', () => {
      ...
      expect(listOfQueues).toEqual(['foo-bar1', 'foo-bar2']);
    });
  });
  ...
  describe('receiveMessage', () => {
    ...
    test('successfully receive message visibility timeout', () => {
      ...
      expect(getQueueState(QueueUrl)).toEqual({
        ...
        messages: [
          {
            '@State': { VisibilityTimeout: 3, isRead: true },
            Attributes: {
              ApproximateFirstReceiveTimestamp: expect.any(Number),
              ApproximateReceiveCount: 1,
              SenderId: 'AAAAAAAAAAAAAAAAAAAAA:i-00a0aaa0aaa000000',
    ...
    test('successfully receive multiple messages', () => {
      ...
      expect(getQueueState(QueueUrl)).toEqual({
        ...
        messages: [
          {
            '@State': { VisibilityTimeout: 2, isRead: true },
            Attributes: {
              ApproximateFirstReceiveTimestamp: expect.any(Number),
              ApproximateReceiveCount: 1,
              SenderId: 'AAAAAAAAAAAAAAAAAAAAA:i-00a0aaa0aaa000000',
    ...
    test('successfully receive message with message single attribute', () => {
      ...
      sendMessage({ MessageAttributes, MessageBody: 'foo1', QueueUrl });
      expect(receiveMessage({ MessageAttributeNames: ['Greeting'], QueueUrl })).toEqual([
        {
          Body: 'foo1',
          MD5OfBody: '299a0be4a5a79e6a59fdd251b19d78bb',
          MD5OfMessageAttributes: 'fd107d7940a40431d0646c5fa93cd140',
          MessageAttributes: {
            Greeting: {
              BinaryValue: 'Hello, World!',
              DataType: 'Binary'
