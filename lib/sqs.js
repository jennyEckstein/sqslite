  ...
  if (queueWithSameName) {
    ...
    compareAttributes.map(attr => {
      if (
        ...
        throw Object.assign(
          new Error(
            `A queue already exists with the same name and a different value for attribute ${attr}`
          ),
          { code: 'QueueAlreadyExists' }
        );
      }
...
function receiveMessage(params) {
  ...
  const messages = queue.messages.slice(0, MaxNumberOfMessages).map(msg => {
    const receivedTimes = msg.Attributes.ApproximateReceiveCount + 1;
    return {
      ...msg,
      '@State': {
        ...msg['@State'],
      ...
      Attributes: {
        ...
        ApproximateReceiveCount: receivedTimes
      }
    };
  });
  ...
  return messages.map(message => {
    const { Attributes, MessageAttributes, MessageBody, MessageId } = message;
    ...
    let attr;
    if (AttributeNames) {
      attr = AttributeNames.includes('All')
        ? Attributes
    ...
    return {
      ...
      ...(AttributeNames && { Attributes: attr }),
      ...(MessageAttributeNames &&
