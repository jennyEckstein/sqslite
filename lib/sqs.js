...
function sendMessage(params) {
  ...
  if (ContentBasedDeduplication) {
    const duplicate = queue.messages.find(
      (message) => !message['@State'].isRead && message.MessageBody === MessageBody
    );
    if (duplicate) {
...
function sendMessageBatch(params) {
  ...
  messages.forEach((message) => {
    const { DelaySeconds, Id, MessageAttributes, MessageBody, MessageGroupId } = message;
    ...
    if (ContentBasedDeduplication) {
      const duplicate = queue.messages.find(
        (msg) => !msg['@State'].isRead && msg.MessageBody === MessageBody
      );
      if (duplicate) {
        AvailableSince = SentTimestamp + CONTENT_DEDUPLICATION_INTERVAL_5_MIN;
      }
