...
function receiveMessage(params) {
  ...
  if (MaxNumberOfMessages && MaxNumberOfMessages > 1) {
    for (let i = 0; i < MaxNumberOfMessages; i += 1) {
      ...
      // TODO:find MaxNumberOfMessages that were not read

      const msg = queue.messages[i];
      if (msg) {
        msg['@State'].isRead = true;
        messages.push(msg);
      }
    }
  } else {
    ...
    if (msg) {
      msg['@State'].isRead = true;
      messages.push(msg); // TODO: make sure that messages exist. This error can occur when no messages were sent to queue, but read from queue was attempted. Test this scenario with AWS
    }
  }

  if (VisibilityTimeout) {
    for (let i = 0; i < messages.length; i += 1) {
      messages[i]['@State'].VisibilityTimeout = VisibilityTimeout;
