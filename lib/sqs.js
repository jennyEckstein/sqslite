...
function sendMessage(params) {
  ...
  // console.log('Queue Send Messages:', queue.messages);
  // console.log('SEND MESSAGE QUEUE:', queue);
...
function receiveMessage(params) {
  ...
  if (VisibilityTimeout) {
    for (let i = 0; i < messages.length; i += 1) {
      ...
      // console.log('queue.readMessages:', queue.readMessages);
      queue.readMessages.push(readMessage);
    }
  } else {
    // console.log('NO VISIBILITY MESSAGES:', queue.readMessages, '|', messages);
  ...
  for (let i = 0; i < messages.length; i += 1) {
    if (MessageAttribute && MessageAttribute !== 'All') {
      if (messages[i].MessageAttributes[MessageAttribute]) {
        // TODO: eval to { BinaryValue: 'Hello, World!', DataType: 'Binary' }, empty obj is true
        const msgAttr = { [MessageAttribute]: messages[i].MessageAttributes[MessageAttribute] };
        messages[i].MessageAttributes = msgAttr;
      }
    }

    Messages.push({
      Body: messages[i].MessageBody,
      MD5OfBody: md5(messages[i].MessageBody),
      ...(MessageAttribute && { MessageAttributes: messages[i].MessageAttributes }),
