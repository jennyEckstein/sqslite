      ...
      describe('RedrivePolicy', () => {
        ...
        test('throw error when creating queue with same name but different deadLetterTargetArn', async () => {
          ...
          const queueParams1 = {
            Attributes: {
              RedrivePolicy:
                '{"deadLetterTargetArn": "arn:aws:sqs:us-east-1:123:dead-letter-queue1","maxReceiveCount": "10"}'
            },
          ...
          const queueParams2 = {
            Attributes: {
              RedrivePolicy:
                '{"deadLetterTargetArn": "arn:aws:sqs:us-east-1:123:dead-letter-queue2","maxReceiveCount": "10"}'
            },
            QueueName: 'foo-bar-123'
          };
        ...
        test('throw error when creating queue with same name but different maxReceiveCount', async () => {
          ...
          const queueParams1 = {
            Attributes: {
              RedrivePolicy:
                '{"deadLetterTargetArn": "arn:aws:sqs:us-east-1:123:dead-letter-queue1","maxReceiveCount": "5"}'
            },
        ...
        test('create queue with Redrive Policy', async () => {
          ...
          const params = {
            Attributes: {
              RedrivePolicy:
                '{"deadLetterTargetArn":"arn:aws:sqs:us-east-1:123:foo-bar-dlq","maxReceiveCount":10}'
            },
            QueueName: 'foo-bar'
          };
    ...
    test('successfully send message', async () => {
      ...
      await expect(
        client
          .sendMessage({
            MessageBody: 'foo',
            QueueUrl: `${endpoint}/123/core-test`
          })
          .promise()
      ).resolves.toEqual({
      ...
      await expect(
        client
          .sendMessage({
            ...
            QueueUrl: `${endpoint}/123/foo-bar123`
          })
          .promise()
